...
#define COAP_TASK_RECV_STACK_SIZE   4096
#define COAP_TASK_SEND_STACK_SIZE   3072
...
typedef struct
{
     ...
     bool isRestore;
     void * client_ptr;
     COAP_SEND_DATA_STRUCT send_data;
}COAP_SEND_Q_MSG;
...
int coap_client_send(UINT32 reqHandle, int coapId, int coapMsgType, int coapMethod, char *ipAddr,
                           int port, int coapPayloadLen, char *coapPayload);
...
int coap_client_option(UINT32 reqHandle, int coapId, int mode, char *optValue);
...
int coap_client_alisign(UINT32 reqHandle, int coapId, char* deviceId, char* deviceName, char*deviceScret, char* productKey, char* signature);
...
void coapCtxStoreNVMem(bool to_clear);
