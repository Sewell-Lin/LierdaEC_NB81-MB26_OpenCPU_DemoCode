...
int64_t security_get_mode(lwm2m_object_t * obj, int instanceId){
    ...
    {
        lwm2m_data_free(size, dataP);
        ECOMM_TRACE(UNILOG_LWM2M, security_get_mode_1, P_INFO, 1, "decode mode=%d",mode);
        return mode;
    }

    lwm2m_data_free(size, dataP);
    fprintf(stderr, "Unable to get security mode : use not secure mode");
    ECOMM_TRACE(UNILOG_LWM2M, security_get_mode_2, P_INFO, 0, "Unable to get security mode : use not secure mode");
    return LWM2M_SECURITY_MODE_NONE;
}

char * security_get_public_id(lwm2m_object_t * obj, int instanceId, int * length){
    ECOMM_TRACE(UNILOG_LWM2M, security_get_public_id_1, P_INFO, 0, "enter");
    int size = 1;
    lwm2m_data_t * dataP = lwm2m_data_new(size);
        ...
        dataP->type == LWM2M_TYPE_OPAQUE)
    {
        ...
        if (buff != 0)
        {
            ...
            *length = dataP->value.asBuffer.length;
            ECOMM_DUMP(UNILOG_LWM2M, security_get_public_id_2, P_INFO, "psk id:",*length,buff);
        }
        lwm2m_data_free(size, dataP);
...
char * security_get_secret_key(lwm2m_object_t * obj, int instanceId, int * length){
    ECOMM_TRACE(UNILOG_LWM2M, security_get_secret_key_1, P_INFO, 0, "enter");
    int size = 1;
    lwm2m_data_t * dataP = lwm2m_data_new(size);
        ...
        dataP->type == LWM2M_TYPE_OPAQUE)
    {
        ...
        if (buff != 0)
        {
            memcpy(buff, dataP->value.asBuffer.buffer, dataP->value.asBuffer.length);
            *length = dataP->value.asBuffer.length;
            ECOMM_DUMP(UNILOG_LWM2M, security_get_secret_key_2, P_INFO, "psk:",*length,buff);
        }
        lwm2m_data_free(size, dataP);
...
#endif
    ECOMM_TRACE(UNILOG_LWM2M, send_data_1, P_INFO, 0, "dtlsconnection::send_data");

    offset = 0;
    while (offset != length)
    {
        ECOMM_TRACE(UNILOG_LWM2M, send_data_2, P_ERROR, 1, "sendto %d bytes", length-offset);
...
static int send_to_peer(struct dtls_context_t *ctx,
        session_t *session, uint8 *data, size_t len, uint8_t raiflag) {

    ECOMM_TRACE(UNILOG_LWM2M, send_to_peer_1, P_INFO, 0, "send_to_peer");
    // find connection
    ...
    if (cnx != NULL)
    {
        ...
        ECOMM_TRACE(UNILOG_LWM2M, send_to_peer_2, P_INFO, 0, "find dtlsconnection call send_data");
        // TODO: nat expiration?
        //  If used this function , remember to apply raiflag here
...
int sockaddr_cmp(struct sockaddr *x, struct sockaddr *y)
{
    ...
    if (x->sa_family == AF_INET)
    {
        ...
        if (y->sa_family == AF_INET)
        {
            ...
            // is V6 mapped V4?
            // Note:NBIOT should not have such IN6 address mapped from IPV4
        } /*else if (ip6_addr_isipv4mappedipv6(&((struct sockaddr_in6 *)y)->sin6_addr)) {
                                 ...
                                 int addressFamily)
{
    ...
    char * host;
    char * port;
    ECOMM_TRACE(UNILOG_LWM2M, dtls_connection_create_0, P_INFO, 0, "enter");
    ...
    if (s >= 0)
    {
        ...
        if (connP != NULL)
        {
            ...
            {
                // no dtls session
                ECOMM_TRACE(UNILOG_LWM2M, dtls_connection_create_1, P_INFO, 0, "no dtls session and free it");
...
int connection_send(dtls_connection_t *connP, uint8_t * buffer, size_t length){
    if (connP->dtlsSession == NULL) {
        // no security
        ECOMM_TRACE(UNILOG_LWM2M, dtlsconnection_send_1, P_INFO, 0, "dtlsconnection::connP->dtlsSession == NULL");
        ...
        {
            ECOMM_TRACE(UNILOG_LWM2M, dtlsconnection_send_2, P_INFO, 0, "timeout need handshake call dtlsconnection_rehandshake");
            // we need to rehandhake because our source IP/port probably changed for the server
            if ( connection_rehandshake(connP, false) != 0 )
            {
                ECOMM_TRACE(UNILOG_LWM2M, dtlsconnection_send_3, P_INFO, 0, "can't send due to rehandshake error");
                printf("can't send due to rehandshake error\n");
                return -1;
            }
        }
        ECOMM_TRACE(UNILOG_LWM2M, dtlsconnection_send_4, P_INFO, 1, "dtlsSession = %x,call dtls_write",connP->dtlsSession);
        if (-1 == dtls_write(connP->dtlsContext, connP->dtlsSession, buffer, length, 0)) {
            ECOMM_TRACE(UNILOG_LWM2M, dtlsconnection_send_5, P_INFO, 0, "dtls_write fail");
            return -1;
        }
    }
...
int connection_handle_packet(dtls_connection_t *connP, uint8_t * buffer, size_t numBytes){
    ...
    if (connP->dtlsSession != NULL)
    {
        ...
        ECOMM_TRACE(UNILOG_LWM2M, dtlsconnection_handle_packet_1, P_INFO, 1, "call dtls_handle_message return:%d ", result);
        if (result !=0) {
...
int connection_rehandshake(dtls_connection_t *connP, bool sendCloseNotify) {
    ...
    if (result !=0) {
         printf("error dtls redtlsconnection %d\n",result);
    }
    return result;
}
                          ...
                          void * userdata)
{
    ...
    if (connP == NULL)
    {
        fprintf(stderr, "#> failed sending %lu bytes, missing dtlsconnection\r\n", length);
        ECOMM_TRACE(UNILOG_LWM2M, lwm2m_buffer_send_1, P_INFO, 0, "lwm2m_buffer_send fail connP == NULL ");
        return COAP_500_INTERNAL_SERVER_ERROR ;
    }

    if (-1 == connection_send(connP, buffer, length))
    {
        ECOMM_TRACE(UNILOG_LWM2M, lwm2m_buffer_send_2, P_INFO, 0, "lwm2m_buffer_send fail dtlsconnection_send == -1 ");
        fprintf(stderr, "#> failed sending %lu bytes\r\n", length);
        return COAP_500_INTERNAL_SERVER_ERROR ;
    }
    ECOMM_TRACE(UNILOG_LWM2M, lwm2m_buffer_send_3, P_INFO, 1, "lwm2m_buffer_send dtlsconnection_send %d", length);
